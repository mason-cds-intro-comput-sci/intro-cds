...
For your convenience, the course website will contain links that take you directly to course content stored on GitHub, and you should be able to access most of your course content using just those links.
However, you may find that you prefer to access your existing content by navigating the GitHub site itself, so let's take a short tour of the GitHub interface.
...
At the top of the dashboard there is a searchbar that can be used to look for content on the GitHub site, such as repositories and user accounts.
Next to the searchbar are four other links: Pull requests, Issues, Marketplace and Explore.
Of these links, the only one you might use for the course is "Pull requests", but we'll save the discussion about Pull requests for another time. 
Next is the bell icon, and clicking it displays your account notifications when someone tags your username, for example.
To the right of that is the plus icon, which is used for creating content such as a repository.

Clicking on your account's profile picture in the upper right opens up another menu containing more links.

```{r navigating-step_3_2, click.img = TRUE}
knitr::include_graphics("img/navigating_step_3_2.jpg")
```

From here you can reach your profile, account settings, the help pages, and a couple of other pages.
...
If you click the *Respositories* tab at the top of your profile, it will display a list of respositories associated with your account, **however keep in mind that it will not have any content you work on for the class**.
...
```{r github-settings-menu, click.img = TRUE}
knitr::include_graphics("img/github_account_settings_menu.png")
```

For now you may want to upload a profile picture or add a short bio under the `Profile` setting, or change how the site sends you email updates and notification alerts under the `Emails` and `Notifications` settings.
...
```{r github-dashboard-dropdown-menu-open, click.img = TRUE}
...
The dashboard is similar to your personal dashboard, but it will only contain content associated with the class.
On the left you will see a list of some of your class repositories over on the left, which you can filter by typing in the `Find a repository...` search box.
If you don't see any repositories there, don't worry, we will get around to creating them soon enough!

To see a full list of your class respositories, similar to the repositories tab on your profile page, click the `View organization` button on the upper right to navigate to the class organization page.
